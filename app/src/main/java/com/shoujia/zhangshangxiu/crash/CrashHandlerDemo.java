package com.shoujia.zhangshangxiu.crash;

/**
 * Created by yuan-zhm on 2018/10/17.
 */
...
public class CrashHandlerDemo implements Thread.UncaughtExceptionHandler {

    private static CrashHandlerDemo sInstance = null;
    private Thread.UncaughtExceptionHandler mDefaultHandler;
    private Context mContext;
    // 保存手机信息和异常信息
    private Map<String, String> mMessage = new HashMap<>();

    public static CrashHandlerDemo getInstance() {
        if (sInstance == null) {
            synchronized (CrashHandlerDemo.class) {
                if (sInstance == null) {
                    synchronized (CrashHandlerDemo.class) {
                        sInstance = new CrashHandlerDemo();
                    }
                }
            }
        }
    ...
    private void saveErrorMessages(Throwable e) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            String path = Environment.getExternalStorageDirectory().getPath() + "/zsx/crash/";
